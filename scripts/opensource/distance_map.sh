...
#     EXEC_PREFIX="${run_type} --nodes=1 -p ${quotatype} -A ${partition} --job-name=${MODEL_TYPE}_${task}  --gres=gpu:$nproc_per_node --gpus=$nproc_per_node --cpus-per-task=32 accelerate launch --num_processes=$nproc_per_node --main_process_port=$master_port --mixed_precision=fp16"           
#     echo ${MODEL_PATH}
